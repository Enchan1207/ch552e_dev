...
static inline bool is_get_descriptor(const usb_setup_packet_t __xdata* packet) {
...
static inline bool is_get_device_descriptor(const usb_setup_packet_t __xdata* packet) {
    return is_get_descriptor(packet) && (packet->wValue >> 8) == USB_DESCRIPTOR_TYPE_DEVICE;
}

static inline bool is_get_configuration_descriptor(const usb_setup_packet_t __xdata* packet) {
    return is_get_descriptor(packet) && (packet->wValue >> 8) == USB_DESCRIPTOR_TYPE_CONFIGURATION;
}

static inline bool is_set_address(const usb_setup_packet_t __xdata* packet) {
...
bool usb_ep0_handle_setup(usb_ep0_ctx_t* ctx, const usb_setup_packet_t __xdata* packet) {
    ...
    if (is_get_device_descriptor(packet)) {
        ...
        memcpy_code_to_xdata(ep0_buffer, descriptor, sizeof(usb_device_descriptor_t));
